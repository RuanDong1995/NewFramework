...
public abstract class BaseFragment<T extends BaseViewModel> extends Fragment {
    ...
    protected void setupViewModel(@NonNull T viewModel) {
        ...
        // 用户登录状态改变提示
        App app = mViewModel.getApplication();
        app.getEventManager().getLoginUserEvent().observeLoginUser(this, new Observer<LoginUserEvent.LoginUser>() {
            @Override
            public void onChanged(@Nullable LoginUserEvent.LoginUser loginUser) {
                onLoginUserChanged(loginUser);
            }
        });
        // 网络状态改变
        app.getEventManager().getNetworkStatusEvent().observeNetworkStatus(this, new Observer<Integer>() {
            @Override
